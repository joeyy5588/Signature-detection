...
class SINGLEGANTrainer:    
    ...
    def _train_dis(self, origin_img, hw_img, pt_img):
        ...
        real_img_2 = torch.cat((pt_img, white_img), dim=1)

        fake_img_3 = torch.cat((hw_img, fake_hw_2), dim=1)
        real_img_3 = torch.cat((hw_img, hw_img), dim=1)
        #print(fake_hw.size(), fake_img.size())

        real_predict = self.dis(real_img)
        fake_predict = self.dis(fake_img)
        # real_predict_2 = self.dis(real_img_2)
        # fake_predict_2 = self.dis(fake_img_2)
        real_predict_3 = self.dis(real_img_3)
        fake_predict_3 = self.dis(fake_img_3)
        ...
        loss_d = (real_loss + fake_loss + real_loss_3 + fake_loss_3) / 4
        loss_d.backward()
    ...
    def _train_gen(self, origin_img, hw_img, pt_img):
        ...
        loss_g = (gen_loss +  gen_loss_3 + hw_loss_3) / 3
